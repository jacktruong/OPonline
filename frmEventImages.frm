...
Begin VB.Form frmEventImages 
   Caption         =   "Event Images"
   ClientHeight    =   3975
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6330
   ...
   End
End
Attribute VB_Name = "frmEventImages"
...
Set dbRec = db.OpenRecordset("SELECT * FROM Events WHERE Events.ID=" & Trim(Str(lstCharacters.ItemData(lstCharacters.ListIndex))) & ";", dbOpenDynaset)
...
Set db = OpenDatabase(App.Path & "\Overpower.mdb")
Set dbRec = db.OpenRecordset("SELECT * FROM Events Order by Events.ID;", dbOpenDynaset)
...
Print #x, "SELECT * FROM Events WHERE Events.id=" & Trim(Str(lstCharacters.ItemData(lstCharacters.ListIndex))) & ";"
Close #x
