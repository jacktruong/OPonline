...
Begin VB.Form frmDoubleshotimages 
   Caption         =   "Doubleshot Images"
   ClientHeight    =   4125
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   8730
   ...
   End
End
Attribute VB_Name = "frmDoubleshotimages"
...
Set dbRec = db.OpenRecordset("SELECT * FROM Doubleshot WHERE Doubleshot.ID=" & Trim(Str(lstCharacters.ItemData(lstCharacters.ListIndex))) & ";", dbOpenDynaset)
...
Set db = OpenDatabase(App.Path & "\Overpower.mdb")
Set dbRec = db.OpenRecordset("SELECT * FROM Doubleshot Order by Doubleshot.ID;", dbOpenDynaset)
...
Title = "DOUBLESHOT: " & Trim(Str(nValue)) & sType1 & "/" & Trim(Str(nValue2)) & sType2 & " [" & sPictured & "]"
...
Print #x, "SELECT * FROM Doubleshot WHERE Doubleshot.id=" & Trim(Str(lstCharacters.ItemData(lstCharacters.ListIndex))) & ";"
Close #x
