...
Begin VB.Form frmArtifactImages 
   Caption         =   "Form1"
   ClientHeight    =   4050
   ClientLeft      =   60
   ClientTop       =   345
   ...
   End
End
Attribute VB_Name = "frmArtifactImages"
...
Set dbRec = db.OpenRecordset("SELECT * FROM Artifact WHERE Artifact.ID=" & Trim(Str(lstCharacters.ItemData(lstCharacters.ListIndex))) & ";", dbOpenDynaset)
...
Set db = OpenDatabase(App.Path & "\Overpower.mdb")
Set dbRec = db.OpenRecordset("SELECT * FROM Artifact Order by Artifact.ID;", dbOpenDynaset)
...
Print #x, "SELECT * FROM Artifact WHERE Artifact.id=" & Trim(Str(lstCharacters.ItemData(lstCharacters.ListIndex))) & ";"
Close #x
