...
Begin VB.Form frmAspectImages 
   Caption         =   "Aspect Images"
   ClientHeight    =   4005
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6555
   ...
   End
End
Attribute VB_Name = "frmAspectImages"
...
Set dbRec = db.OpenRecordset("SELECT * FROM Aspect WHERE Aspect.ID=" & Trim(Str(lstCharacters.ItemData(lstCharacters.ListIndex))) & ";", dbOpenDynaset)
...
Set db = OpenDatabase(App.Path & "\Overpower.mdb")
Set dbRec = db.OpenRecordset("SELECT * FROM Aspect Order by Aspect.ID;", dbOpenDynaset)
...
Print #x, "SELECT * FROM Aspect WHERE Aspect.id=" & Trim(Str(lstCharacters.ItemData(lstCharacters.ListIndex))) & ";"
Close #x
